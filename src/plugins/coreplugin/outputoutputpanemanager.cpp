...
/*
void OutputOutputPaneManager::updateStatusButtons(bool visible)
{
...
void OutputOutputPaneManager::init()
{
    ...
    */
    ...
    //connect(m_pane, SIGNAL(showPage(int)), this, SLOT(showPage(int)));
/*
    int shortcutNumber = 1;
    for (int i = 0; i != n; ++i) {
        IOutputOutputPane *outPane = m_panes.at(i);
...
/*
void OutputOutputPaneManager::shortcutTriggered()
{
    QAction *action = qobject_cast<QAction*>(sender());
    QTC_ASSERT(action, return);
    int idx = m_actions.indexOf(action);
    QTC_ASSERT(idx != -1, return);
    Core::IOutputOutputPane *outputPane = m_panes.at(idx);
    ...
    if (OutputOutputPanePlaceHolder::isCurrentVisible() && current == idx) {
...
/*
void OutputOutputPaneManager::buttonTriggered()
{
    OutputPaneToggleButton *button = qobject_cast<OutputPaneToggleButton *>(sender());
    buttonTriggered(m_buttons.indexOf(button));
}
void OutputOutputPaneManager::buttonTriggered(int idx)
{
    QTC_ASSERT(idx >= 0, return);
    if (idx == currentIndex() && OutputOutputPanePlaceHolder::isCurrentVisible()) {
        // we should toggle and the page is already visible and we are actually closeable
        slotHide();
    } else {
        showPage(idx, IOutputOutputPane::ModeSwitch | IOutputOutputPane::WithFocus);
    }
}
*/
...
/*
void OutputOutputPaneManager::slotNext()
{
    int idx = currentIndex();
    ensurePageVisible(idx);
    IOutputOutputPane *out = m_panes.at(idx);
    if (out->canNext())
        out->goToNext();
}

void OutputOutputPaneManager::slotPrev()
{
    int idx = currentIndex();
    ensurePageVisible(idx);
    IOutputOutputPane *out = m_panes.at(idx);
...
void OutputOutputPaneManager::slotHide()
{
    OutputOutputPanePlaceHolder *ph = OutputOutputPanePlaceHolder::getCurrent();
    if (ph) {
        ph->setVisible(false);
...
int OutputOutputPaneManager::findIndexForPage(IOutputOutputPane *out)
{
...
*/
/*
void OutputOutputPaneManager::updateNavigateState()
{
    IOutputOutputPane *pane = qobject_cast<IOutputOutputPane*>(sender());
...
void OutputOutputPaneManager::flashButton()
{
    IOutputOutputPane* pane = qobject_cast<IOutputOutputPane*>(sender());
    int idx = findIndexForPage(pane);
    if (pane)
...
void OutputOutputPaneManager::setBadgeNumber(int number)
{
    IOutputOutputPane* pane = qobject_cast<IOutputOutputPane*>(sender());
    int idx = findIndexForPage(pane);
    if (pane)
...
void OutputOutputPaneManager::showPage(int idx, int flags)
{
    QTC_ASSERT(idx >= 0, return);
    OutputOutputPanePlaceHolder *ph = OutputOutputPanePlaceHolder::getCurrent();

    if (!ph && flags & IOutputOutputPane::ModeSwitch) {
        // In this mode we don't have a placeholder
        // switch to the output mode and switch the page
        ModeManager::activateMode(Id(Constants::MODE_EDIT));
        ph = OutputOutputPanePlaceHolder::getCurrent();
    }
    ...
    if (ph) {
        ...
        if (flags & IOutputOutputPane::WithFocus && out->canFocus())
            out->setFocus();

        if (flags & IOutputOutputPane::EnsureSizeHint)
...
void OutputOutputPaneManager::togglePage(int flags)
{
    int idx = findIndexForPage(qobject_cast<IOutputOutputPane*>(sender()));
    if (OutputOutputPanePlaceHolder::isCurrentVisible() && currentIndex() == idx)
         slotHide();
    else
         showPage(idx, flags);
}

void OutputOutputPaneManager::focusInEvent(QFocusEvent *e)
{
    if (QWidget *w = m_outputWidgetPane->currentWidget())
        w->setFocus(e->reason());
}

void OutputOutputPaneManager::setCurrentIndex(int idx)
{
    ...
    if (idx != -1) {
        ...
        IOutputOutputPane *pane = m_panes.at(idx);
        ...
        m_buttons.at(idx)->setChecked(OutputOutputPanePlaceHolder::isCurrentVisible());
...
*/
/*
void OutputOutputPaneManager::popupMenu()
{
    QMenu menu;
    QAction *act = menu.addAction(m_pane->displayName());
    ...
    if (button->isVisible()) {
        ...
        button->show();
        showPage(idx, IOutputOutputPane::ModeSwitch);
    }
}
*/
